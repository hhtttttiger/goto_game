...
const std::wstring APPTITLE = L"Transparent Sprite Demo";
...
bool Game_Init(HWND window)
{
    ...
    {
        MessageBox(0, L"Error initializing Direct3D",L"ERROR",0);
        return false;
    }
    ...
    {
        MessageBox(0, L"Error initializing DirectInput",L"ERROR",0);
        return false;
    }

    //load non-transparent image
    image_notrans = LoadTexture(L"shuttle_notrans.bmp");
    if (!image_notrans) return false;

    //load color-keyed transparent image
    image_colorkey = LoadTexture(L"shuttle_colorkey.bmp", D3DCOLOR_XRGB(255,0,255));
    if (!image_colorkey) return false;

    //load alpha transparent image
    image_alpha = LoadTexture(L"shuttle_alpha.tga");
    if (!image_alpha) return false;


    //You can use this function to get the size of the image
    D3DXVECTOR2 size = GetBitmapSize(L"shuttle_alpha.tga");
